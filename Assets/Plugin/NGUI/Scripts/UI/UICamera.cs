...
public class UICamera : MonoBehaviour
{
	...
	public static ProcessEventsIn processEventsIn = ProcessEventsIn.Update;
    ...
    /// </summary>
    public static bool s_bEventProcessed = false;//自己添加的
    static public void Notify (GameObject go, string funcName, object obj)
	{
		if (mNotifying > 10) return;

        UICamera.s_bEventProcessed = false;//自己添加的
        ...
        */
        if (go && go.activeInHierarchy)
        {
            ...
            if (mGenericHandler != null && mGenericHandler != go)
                mGenericHandler.SendMessage(funcName, obj, SendMessageOptions.DontRequireReceiver);
           /* if (go && XUIObjectBase.ActionUIEventFilter != null && !UICamera.s_bEventProcessed)
            {
                XUIObject component = go.GetComponent<XUIObject>();
                if (null != component && (funcName == "OnClick" || funcName == "OnPress" || funcName == "OnDoubleClick" || funcName == "OnDrag") && XUIObjectBase.ActionUIEventFilter(component, funcName))
                {
                    Debug.Log("UIcamera:reture");
                    return;
                }
            }
            */
            if (go && !UICamera.s_bEventProcessed && mGenericHandler != go)
            {
                go.SendMessage(funcName, obj, SendMessageOptions.DontRequireReceiver);
            }
            --mNotifying;
